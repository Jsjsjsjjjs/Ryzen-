...
export default function Home() {
  // Fetched live from DB via bot Express API.
  // Known rewards → existing logo/gradient. Unknown names → default 🎁 / indigo gradient.
      ...
      <div className="container mx-auto px-4 py-16 md:py-24">
        ...
        <div className="flex flex-col items-center text-center space-y-8 max-w-3xl mx-auto">
          ...
          <h1 className="text-5xl md:text-7xl font-extrabold tracking-tight bg-clip-text text-transparent bg-gradient-to-br from-white to-white/50">
            Claim Your{" "}
            <span className="text-primary drop-shadow-[0_0_15px_rgba(88,101,242,0.5)]">
              Exclusive
            </span>{" "}
            Rewards
          </h1>
          ...
          <p className="text-lg text-muted-foreground max-w-2xl leading-relaxed">
            Welcome to the elite tier of Discord rewards. Invite your friends,
            earn invites, and unlock premium gaming subscriptions and gift cards.
          </p>

          <div className="flex flex-wrap gap-4 justify-center pt-4">
            <Link
              href="/claim"
              className="inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring bg-primary text-primary-foreground shadow-[0_0_20px_-5px_rgba(88,101,242,0.6)] hover:bg-primary/90 h-12 px-8 py-2"
            >
              Redeem Code <ArrowRight className="ml-2 h-4 w-4" />
            </Link>
            <Link
              href="/rewards"
              className="inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring border border-white/10 bg-transparent hover:bg-white/5 h-12 px-8 py-2"
            >
              View All Rewards
            </Link>
          </div>
        ...
        <div className="mt-24 max-w-2xl mx-auto">
          <div className="p-8 rounded-2xl border border-white/10 bg-card/60 backdrop-blur-xl shadow-2xl relative overflow-hidden">
            ...
            <form
              ...
              onSubmit={(e) => {
                e.preventDefault();
                const code = (e.target as any).elements.code.value;
                if (code) window.location.href = `/claim?code=${code}`;
              }}
            >
              <input
                name="code"
        ...
        {/* Reward Tiers — live from DB */}
        <div className="mt-32 max-w-6xl mx-auto text-center">
          <h2 className="text-3xl font-bold mb-3">Reward Tiers</h2>
          <p className="text-muted-foreground mb-12">
            Invite more to unlock bigger rewards
          </p>

          {isLoading ? (
            // Skeleton placeholders — same 2×4 grid, no layout shift
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {Array.from({ length: 8 }).map((_, i) => (
                <Skeleton
                  key={i}
                  className="h-36 rounded-xl bg-card/60 border border-white/5"
                />
              ))}
            </div>
          ) : (
              ...
              {rewards?.map((reward) => (
                <div
                  ...
                  className={`relative rounded-xl border border-white/10 bg-gradient-to-br ${
                    // Known rewards get their exact gradient; new DB entries fall back gracefully
                    REWARD_BG_COLORS[reward.name] ?? "from-primary/10 to-primary/5"
                  } p-5 text-left overflow-hidden group hover:border-white/20 hover:scale-[1.02] transition-all`}
                >
                  ...
                  <div className="relative z-10">
                    <div className="flex items-start justify-between mb-3">
                      <div className="h-12 w-12 rounded-lg bg-black/40 border border-white/10 flex items-center justify-center group-hover:scale-110 transition-transform">
                        {/* Known rewards → brand SVG icon. New rewards → 🎁 fallback */}
                    ...
                    <p className="text-xs text-white/50 mt-1">
                      {reward.invitesRequired} invites needed
                    </p>
                  </div>
                </div>
              ))}
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
            <div className="relative p-6 rounded-2xl border border-white/5 bg-card/50 backdrop-blur hover:bg-card transition-colors">
              ...
              <p className="text-muted-foreground">
                Use your Discord invite link and get people to join. Type{" "}
                <code className="bg-white/5 px-1 rounded text-primary">-i</code>{" "}
                in Discord to check your invite count.
              </p>
            </div>
            <div className="relative p-6 rounded-2xl border border-white/5 bg-card/50 backdrop-blur hover:bg-card transition-colors">
              <div className="h-12 w-12 rounded-lg bg-primary/20 flex items-center justify-center mb-6">
              ...
              <p className="text-muted-foreground">
                Once you hit the required invites, the bot will show eligible
                rewards. Pick one and then do{" "}
                <code className="bg-white/5 px-1 rounded text-primary">-rmi</code>.
              </p>
            </div>
            <div className="relative p-6 rounded-2xl border border-white/5 bg-card/50 backdrop-blur hover:bg-card transition-colors">
              <div className="h-12 w-12 rounded-lg bg-primary/20 flex items-center justify-center mb-6">
              ...
              <p className="text-muted-foreground">
                Staff verifies and DMs you the code. Redeem it right here on this
                website using the code you receive.
              </p>
            </div>
          </div>
        </div>
